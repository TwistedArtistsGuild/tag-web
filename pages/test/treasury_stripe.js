...
const simulateStripeCharge = () => {
const chargeId = `ch_${Date.now()}_sim`
setStripeChargeId(chargeId)
}
...
<div className="divider text-xs">Stripe Payment Simulation</div>

<div className="flex flex-wrap gap-3 items-end mb-4">
<label className="form-control">
<span className="label-text text-sm">Test Card</span>
<select className="select select-bordered select-sm" value={stripeTestCard} onChange={(e) => setStripeTestCard(e.target.value)}>
{Object.entries(stripeTestCards).map(([card, label]) => (
<option key={card} value={card}>{label} ({card.slice(-4)})</option>
))}
</select>
</label>
<button
...
onClick={simulateStripeCharge}
disabled={stripeChargeId !== ""}
>
{stripeChargeId ? "✓ Charged" : "Simulate Charge"}
</button>
{stripeChargeId && (
<div className="text-xs opacity-70">
ID: <span className="font-mono">{stripeChargeId.slice(-8)}</span>
</div>
)}
</div>
...
<div className="card-body">
<h2 className="font-semibold text-lg">💳 Stripe Charge Simulation</h2>
<div className="grid grid-cols-2 md:grid-cols-4 gap-3">
<div className="bg-base-200 rounded p-3">
<div className="text-xs opacity-60">Charge ID</div>
<div className="text-sm font-mono text-ellipsis">{stripeChargeId}</div>
</div>
<div className="bg-base-200 rounded p-3">
<div className="text-xs opacity-60">Test Card</div>
<div className="text-sm">{stripeTestCards[stripeTestCard]}</div>
<div className="text-xs opacity-60">{stripeTestCard.slice(-4)}</div>
</div>
<div className="bg-base-200 rounded p-3">
<div className="text-xs opacity-60">Amount</div>
<div className="text-sm font-semibold">${(receiptTotals.grossCents / 100).toFixed(2)} {currency}</div>
</div>
<div className="bg-base-200 rounded p-3">
...
<p className="text-xs opacity-60 mb-2">All purchased items and the total charged to their payment method.
{stripeChargeId && (
<span className="ml-2">• Charge: <span className="font-mono">{stripeChargeId.slice(0, 8)}...{stripeChargeId.slice(-4)}</span></span>
)}
</p>
